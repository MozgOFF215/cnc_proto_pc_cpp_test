...
*/
  ...
  */
  ...
  DCB port_settings = {0}; // Initializing DCB structure
  port_settings.DCBlength = sizeof(port_settings);
  if (!GetCommState(Cport[comport_number], &port_settings))
  {
    printf("unable to get comport dcb settings\n");
    CloseHandle(Cport[comport_number]);
    return (1);
  }
  port_settings.BaudRate = CBR_9600;   // Setting BaudRate = 9600
  port_settings.ByteSize = 8;          // Setting ByteSize = 8
  port_settings.StopBits = ONESTOPBIT; // Setting StopBits = 1
  port_settings.Parity = NOPARITY;     // Setting Parity = None
